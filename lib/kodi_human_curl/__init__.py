"""
## Multiple platform selector for human_curl binary distributions
# Original from: https://pypi.python.org/pypi/human_curl/0.1.9

## To Use:
# in addon.xml
<import addon="script.module.human_curl" version="0.1.9"/>
# in your project
import kodi_human_curl
import human_curl
...
cd script.module.human_curl/lib
python -c "import kodi_human_curl; kodi_human_curl.build_binary_dist()"
...
if not binary_package_path or not os.path.exists(binary_package_path):
    ...
    # Load real module
    import imp
    # imp.load_source(package, os.path.join(binary_package_path, 'human_curl','__init__.py'))
    curdir = os.path.abspath(os.curdir)
    fh = None
    try:
        os.chdir(os.path.join(binary_package_path))
        fh, pathname, description = imp.find_module(package)
        imp.load_module(package, fh, pathname, description)
    finally:
        if fh:
            fh.close()
        os.chdir(curdir)
